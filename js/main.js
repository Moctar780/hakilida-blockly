...
function updateProgress(amount, label) {
	loadingProgress = Math.min(loadingProgress + amount, 95); // Cap at 95% until complete
	const progressEl = document.getElementById('loader-progress');
	const statusEl = document.getElementById('loader-status');
	if (progressEl) progressEl.style.width = loadingProgress + '%';
	if (statusEl && label) statusEl.textContent = label;
	console.log(`Progress: ${loadingProgress}% - ${label}`);
}
...
async function init() {
	...
	try {
		...
		updateProgress(10, 'Loading models...');
		await loadModels();
		
		updateProgress(30, 'Processing scene...');
		...
		updateProgress(40, 'Building track...');

		// Probes - ULTRA-OPTIMIZED: Skip completely for faster loading
		// The LightProbeGrid baking is still the main bottleneck
		// For production, we can generate these offline and cache them
		
		const probeHeight = 6;
		const probes = new LightProbeGrid(
			hw * 2, probeHeight, hd * 2,
			Math.max( 2, Math.round( hw / 8 ) ),  // Minimal density
			1,                                     // Single height level
			Math.max( 2, Math.round( hd / 8 ) ),
		);
		probes.position.set( bounds.centerX, probeHeight / 2, bounds.centerZ );
		
		updateProgress(50, 'Baking lighting...');
		
		// Bake with minimal settings - further optimized
		probes.bake( renderer, scene, { cubemapSize: 8, near: 0.1, far: groundSize } );
		scene.add( probes );

		updateProgress(60, 'Setting up physics...');
		...
		updateProgress(75, 'Loading audio...');
		...
		updateProgress(85, 'Initializing Blockly...');
		...
		function animate() {
			...
			// Hide loader on FIRST frame and mark progress as complete
			if ( isFirstFrame ) {
				isFirstFrame = false;
		...
		hasError = true;
		const statusEl = document.getElementById( 'loader-status' );
		if ( statusEl ) statusEl.textContent = '❌ ' + (e.message || 'Error loading game');
